...
public class QuestionnaireController {
    ...
    public ModelAndView showUpdateQuestionnairForm(@PathVariable("id") String id) {
        Optional<Questionnaire> questionnaire = questionnaireRepository.findById(id);
        if (questionnaire.isPresent()) {
    ...
    @PutMapping("/{id}")
    public ModelAndView updateQuestionnaire(@PathVariable("id") String id, @Valid Questionnaire questionnaire, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            return new ModelAndView("update");
        }

        Optional<Questionnaire> questionnaire1 = questionnaireRepository.findById(id);
        if (questionnaire1.isPresent()) {
            questionnaireRepository.save(questionnaire);
            return new ModelAndView("redirect:/questionnaires");
        } else {
            return new ModelAndView("404");
